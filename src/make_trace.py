...
sorted_history = buyer_history[["buyer_id","timestamp","event"]].sort_values(["buyer_id","timestamp"],ascending=True)
...
with open(cmd_folder+"data/processed/trace.json","w") as buf:
    ...
    for buyer in buyers[0:buyers.size-1]:
        ...
        sorted_history[sorted_history["buyer_id"]==buyer][["event"]].to_json(path_or_buf=buf,orient="records",force_ascii=False)
        buf.write("},\n")
    buf.write("{\"id\":\""+buyer+"\",\"trace\":")
    sorted_history[sorted_history["buyer_id"]==buyer][["event"]].to_json(path_or_buf=buf,orient="records",force_ascii=False)
    buf.write("}]")
